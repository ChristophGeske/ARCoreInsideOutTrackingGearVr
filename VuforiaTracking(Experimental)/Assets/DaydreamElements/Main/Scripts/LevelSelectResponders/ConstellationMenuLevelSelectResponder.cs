...
namespace DaydreamElements.Main {
  ...
  /// Used to disable Constellation Menu when the navigation menu is open
  public class ConstellationMenuLevelSelectResponder : BaseLevelSelectResponder {
    private ConstellationMenuRoot constellationMenu;
    private GvrTrackedController controller;
    private GvrLaserPointer pointer;

    public override void OnMenuOpened() {
      SetConstellationMenuEnabled(false);
    }

    public override void OnMenuClosed() {
      SetConstellationMenuEnabled(true);
    }
    ...
    private void SetConstellationMenuEnabled(bool enabled) {
      constellationMenu.gameObject.SetActive(enabled);
