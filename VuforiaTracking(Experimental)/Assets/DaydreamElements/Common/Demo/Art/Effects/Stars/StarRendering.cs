...
public class StarRendering : MonoBehaviour {
  ...
  public static readonly Vector3[] quadVertices = 
    new Vector3[4]{ new Vector3(-0.5f, -0.5f, 0),
  ...
  public static readonly Vector2[] quadUV = new Vector2[4]{
                                            new Vector2(0, 0),
                                            new Vector2(1, 0), 
                                            new Vector2(0, 1), 
                                            new Vector2(1,1)};
  ...
  static Vector3 GetPosition(float distance, float yaw, float pitch) {
    return distance*new Vector3(Mathf.Cos(Mathf.Deg2Rad * pitch) *
                        Mathf.Sin(Mathf.Deg2Rad * yaw),
                        Mathf.Sin(Mathf.Deg2Rad * pitch),
                        Mathf.Cos(Mathf.Deg2Rad * pitch) *
                         Mathf.Cos(Mathf.Deg2Rad * yaw));
  }
  ...
  void UpdateStars() {
    ...
    for(int c=0; c<constellations.Length; c++){
      ...
      for(int i=0; i<con.stars.Length; i++){
        ...
        vertices[4*idx + 0] = Quaternion.AngleAxis(yaw, Vector3.up) *
                              Quaternion.AngleAxis(-pitch, Vector3.right) *
                              (s.scale*quadVertices[0]) + position;
        vertices[4*idx + 1] = Quaternion.AngleAxis(yaw, Vector3.up) *
                              Quaternion.AngleAxis(-pitch, Vector3.right) *
                              (s.scale*quadVertices[1]) + position;
        vertices[4*idx + 2] = Quaternion.AngleAxis(yaw, Vector3.up) *
                              Quaternion.AngleAxis(-pitch, Vector3.right) *
                              (s.scale*quadVertices[2]) + position;
        vertices[4*idx + 3] = Quaternion.AngleAxis(yaw, Vector3.up) *
                              Quaternion.AngleAxis(-pitch, Vector3.right) *
                              (s.scale*quadVertices[3]) + position;
      }
    ...
    for(int i=0; i<randomStars.Length; i++) {
      ...
      vertices[4*idx + 0] = Quaternion.AngleAxis(s.yaw, Vector3.up) *
                            Quaternion.AngleAxis(-s.pitch, Vector3.right) *
                            (s.scale*quadVertices[0]) + position;
      vertices[4*idx + 1] = Quaternion.AngleAxis(s.yaw, Vector3.up) *
                            Quaternion.AngleAxis(-s.pitch, Vector3.right) *
                            (s.scale*quadVertices[1]) + position;
      vertices[4*idx + 2] = Quaternion.AngleAxis(s.yaw, Vector3.up) *
                            Quaternion.AngleAxis(-s.pitch, Vector3.right) *
                            (s.scale*quadVertices[2]) + position;
      vertices[4*idx + 3] = Quaternion.AngleAxis(s.yaw, Vector3.up) *
                            Quaternion.AngleAxis(-s.pitch, Vector3.right) *
                            (s.scale*quadVertices[3]) + position;
    }
